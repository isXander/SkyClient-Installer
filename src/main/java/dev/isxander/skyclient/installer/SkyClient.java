...
public class SkyClient {
    ...
    public void install() throws IOException {
        Log.startSection("Attempting to install SkyClient...");

        RepositoryManager repo = getRepositoryManager();

        Log.startSection("Checking possibility of SkyClient install...");
        if (!mcDir.exists()) {
        ...
        File scDir = getScDir();
        if (scDir.exists()) {
            Log.startSection("SkyClient is already installed. Warning user.");
            int option = JOptionPane.showConfirmDialog(null, "Your SkyClient directory already exists. All data in the SkyClient folder will be deleted.\nDo you want to continue anyway?", "SkyClient is already installed", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
            if (option == JOptionPane.YES_OPTION) {
                Log.sendSectionInfo("User permitted deletion of SkyClient directory. Deleting.");
                FileUtils.deleteDirectory(scDir);
                Log.sendSectionInfo("Finished deleting SkyClient.");
            } else {
                Log.sendSectionInfo("User did not accept deletion of SkyClient. Aborting install.");
        ...
        Log.startSection("Installing Packs...");
        for (PackEntry pack : repo.getPackEntries()) {
            if (!pack.isEnabled()) continue;

            Log.startSection("Downloading Pack: " +  pack.getDisplayName());
            if (!pack.getDownloadUrl().startsWith("https://")) {
                Log.endSection(pack.getDisplayName() + " tried to download invalid URL: " + pack.getDownloadUrl());
                continue;
            }
        ...
        Log.info("Finished installing SkyClient!");
    }
